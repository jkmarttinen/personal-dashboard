...
// Static Dashboard Logic

// Embedded data will be injected before this script as 'embeddedData' constant
// window.calendarData is populated from embeddedData.calendarData
// audioBase64 is populated from embeddedData.audioBase64

window.calendarData = embeddedData.calendarData;
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (holiday) {
            ...
            holDiv.textContent = Array.isArray(holiday) ? holiday.join(', ') : holiday;
        ...
        if (nameday) {
            ...
            nameDiv.textContent = Array.isArray(nameday) ? nameday.join(', ') : nameday;
...
function getWeatherIcon(code) {
    ...
    if (val === 14 || val === 15) return icons.cloudRain;
    ...
    if (val >= 80 && val <= 99) return icons.cloudRain;
...
async function fetchWeather() {
    ...
    try {
        // Direct fetch to Digitraffic API
        const response = await fetch('https://tie.digitraffic.fi/api/weather/v1/stations/1087/data');
...
updateClock();
renderCalendar();
fetchWeather(); // Requires internet and CORS support from API

// Intervals
setInterval(updateClock, 1000);
setInterval(renderCalendar, 60 * 60 * 1000); // Check date change hourly
